...
const Footer: FunctionComponent<IFooterRouter & IFooterProps> = ({router}) => {
  ...
  return (<footer>
    <Navbar color="dark" fixed="bottom" dark>
      {timerRunning ? <FooterDurationTimer/> : <NavbarBrand href="#" onClick={() => navigateToRoute(RouteNames.ROOT)} className="mr-auto text-white-50">repsyfi</NavbarBrand>}
        ...
        <Nav navbar>
          <NavItem>
            <Row className="text-center">
              <Col>
                <NavLink href="#" onClick={() => navigateToRoute(RouteNames.ROOT)}>{t("Home")}</NavLink>
              </Col>
              <Col>
                <NavLink href="#" onClick={() => navigateToRoute(RouteNames.ALL_DAYS)}>{t("All days")}</NavLink>
              </Col>
            </Row>
          </NavItem>
